...
import (
	...
	"github.com/oneiro-ndev/tendermint.0.32.3/libs/fail"
	...
	cstypes "github.com/oneiro-ndev/tendermint.0.32.3/consensus/types"
	tmevents "github.com/oneiro-ndev/tendermint.0.32.3/libs/events"
...
func (cs *ConsensusState) handleMsg(mi msgInfo) {
	...
	case *VoteMessage:
		...
		// a typical peer.
		// https://github.com/oneiro-ndev/tendermint.0.32.3/issues/1281
		// }
	...
	if err != nil { // nolint:staticcheck
		...
		// https://github.com/oneiro-ndev/tendermint.0.32.3/issues/3406
...
func (cs *ConsensusState) tryAddVote(vote *types.Vote, peerID p2p.ID) (bool, error) {
	...
	if err != nil {
		...
		if err == ErrVoteHeightMismatch {
			...
			// 3) tmkms use with multiple validators connecting to a single tmkms instance (https://github.com/oneiro-ndev/tendermint.0.32.3/issues/3839).
...
func (cs *ConsensusState) addVote(vote *types.Vote, peerID p2p.ID) (added bool, err error) {
	...
	case types.PrecommitType:
		...
		cs.Logger.Info(fmt.Sprintf("Debug:..... 1657: TwoThirdsMajority got blockID %v - %v", blockID, ok))
		if ok {
			// Executed as TwoThirdsMajority could be from a higher round
			cs.enterNewRound(height, vote.Round)
			cs.enterPrecommit(height, vote.Round)
			cs.Logger.Info(fmt.Sprintf("Debug:..... 1662: BlockID hash length %v - %v", blockID.Hash, len(blockID.Hash)))
			if len(blockID.Hash) != 0 {
				cs.Logger.Info(fmt.Sprintf("Debug:..... 1664: enter commit with height %v, round %v", height, vote.Round))
				cs.enterCommit(height, vote.Round)
				if cs.config.SkipTimeoutCommit && precommits.HasAll() {
					cs.enterNewRound(cs.Height, 0)
				}
			} else {
				cs.Logger.Info(fmt.Sprintf("Debug:..... 1672: enter precommit wait with height %v, round %v", height, vote.Round))
				cs.enterPrecommitWait(height, vote.Round)
			}
		} else if cs.Round <= vote.Round && precommits.HasTwoThirdsAny() {
			cs.Logger.Info(fmt.Sprintf("Debug:..... 1676: enter new round with height %v, round %v", height, vote.Round))
			cs.enterNewRound(height, vote.Round)
			cs.enterPrecommitWait(height, vote.Round)
		}
